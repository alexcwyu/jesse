...
### 3. Source Layout
- **Layout**: `src/` layout
- **Package moved**: `jesse/` -> `src/jesse/`
- **Import unchanged**: `import jesse` still works

If upstream adds files to the old location, move them to `src/jesse/`.
...
- `joblib>=1.4.0` and `scikit-learn>=1.6.0` were added to `[project.dependencies]` because `jesse/research/ml.py` imports them at runtime but they were not declared.
...
3. **Check source layout**: If upstream adds new modules to the old path, move them to `src/jesse/`.
4. **Re-lock**: Run `uv lock` to update `uv.lock` with any new/changed dependencies.
5. **Verify**: Run `uv sync && uv run python -c "import jesse" && uv run pytest` (if tests exist).

## Quick Commands

```bash
uv sync                                    # Install all deps
uv run python -c "import jesse"            # Verify import
